...
# 기본 3회 재생성 평균 (--runs). 호출 ≈ 3회 × 3~5 · 6~9분. mock 으로는 재지 않는다 (CLAUDE.md §2).
#
#   scripts/graph_bench.sh --tag gbase-20260912                       # 기준선
#   scripts/graph_bench.sh --tag g1-20260912 --compare gbase-20260912 --note "가설 이름"
#   옵션: --stage all|graph|align (align = 저장 그래프 고정, F-11 만) · --runs N (기본 3) · --bundle-dir DIR · --no-rebuild (호출 0)
#
# 마지막 줄이 VERDICT: IMPROVED|REGRESSED|NOISE|BASELINE|INVALID 다. INVALID = 노드 0 (F-06/07 실패).
set -euo pipefail
cd "$(dirname "$0")/.."

TAG="" COMPARE="" NOTE="" BUNDLE_DIR="" REBUILD=1 STAGE=all RUNS=3
while [[ $# -gt 0 ]]; do
  case "$1" in
    --tag) TAG=$2; shift 2 ;;
...
(( REBUILD )) && ARGS+=(--rebuild --stage "$STAGE" --runs "$RUNS")
