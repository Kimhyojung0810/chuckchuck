...
| `items[].confidence` | float | ✅ | 판정 확신도 `0.0~1.0`. 코드가 판정을 뒤집으면 `0.5` 로 낮춘다 |
| `items[].checks` | object | ✅ | 4축 `정의·원리·관계·이유` → bool. 키는 항상 넷 다 있다 |
| `items[].suggestion` | string | ✅ | "이렇게 말해보세요" — 발표에서 그대로 읽을 한 문장. 없으면 `""` |
...
5. **evidence 없는 `contradiction` 은 내보내지 않는다** — 결정적 폴백으로 강등.
   이때 `confidence` 도 `0.5` 로 낮춘다. LLM 이 매긴 확신도는 *뒤집히기 전 판정*에
   대한 값이라, 그대로 두면 화면에 "누락 · 확신도 95%" 같은 거짓말이 뜬다.
6. **`missing` 판정에는 `checks` 가 하나도 켜지지 않는다** — 말한 게 없다는 판정인데
   "정의는 말했다"가 켜져 있으면 판정과 근거가 서로 모순된다. 모델이 채워 보내도 코드가 지운다.
...
| F-12 | LLM JSON 문자열 ×4 (병렬) | `ChatterDoc` | `f12_chatter.py` |
...
| `chuckchuck/f12_chatter.py` | ConceptGraph+AlignmentDoc+FlowDiff → ChatterDoc (국내 LLM ×4) |
