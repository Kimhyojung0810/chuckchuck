...
|---|---|---|---|---|---|
| 기본 (`rubric=true, judge=false`) | 1 | 3 (중앙값) | 0 | 0 | **4** |
| `rubric=false` | 1 | 0 | 0 | 0 | 1 |
| `judge=true, limit=2` | 1 | 3 | 2 × 4벌 = 8 | 0 | **12** |
| `--coach` 까지 | 1 | 3 | 8 | 2 × 2단 = 4 | 16 |

기준선 1회 + 변형 N회. 기본 설정 3변형이면 호출 16회, 판정까지 켜면 48회. rubric 은 3번 재서 중앙값 — 같은 질문에 한 번 채점은 ±0.7 흔들린다 (09-12 실측). **`maxVariants` 상한은 5** (스크립트가 자른다).
...
- 잡음 폭은 **같은 코드로 3회 재서** 정한다: `examples/qa_eval_noise.py` 가 반복 결과에서 지표별 범위·표준편차를 내고 `PRIMARY` 와 대조한다.
  09-13 실측(Solar · 픽스처 1 · 질문 2): 특이도 ±1.0, 인용률 ±0.2, 골자 통과 ±0.5, rubric ±0.5(전 항목 범위 0). 그 전 값(±0.5·±0.05)은 짐작이었고,
  그 폭으로 찍힌 장부의 판정 — 유일한 IMPROVED 포함 — 은 잡음 안이다. A.X 는 rubric 종합이 1.26 흔들려 판정에 못 쓴다.
  **표본이 1건인 동안 이 루프는 대부분 NOISE 를 낸다. 그게 맞는 답이다.** 번들이 5건 넘으면 다시 잰다.
- **그래프 단계는 더 흔들린다.** 09-12 밤, 같은 코드로 3회: 노드 7/9/11 · 요약 근거 0.30/0.43/0.55 · 고립 0/0/0.18.
  그래서 `graph_bench.sh` 는 3회 재생성 평균이 기본(`--runs`), 폭은 그 실측에서 정했다 (`graph_eval.PRIMARY` 주석). 변형당 6~9분.
