...
| 옵션 | 질문 생성 | 판정 | 코칭 | 합계 |
|---|---|---|---|---|
| 기본 (`judge=false`) | 1 | 0 | 0 | **1** |
| `judge=true, limit=2` | 1 | 2 × 4벌 = 8 | 0 | **9** |
| `--coach` 까지 | 1 | 8 | 2 × 2단 = 4 | 13 |

기준선 1회 + 변형 N회. 기본 설정 3변형이면 호출 4회, 판정까지 켜면 36회. **`maxVariants` 상한은 5** (스크립트가 자른다).

## 3. 판정 규칙 (`examples/qa_eval_compare.py`)

- 1차 지표 15개: 특이도·인용률·폴백·말투 3종·판정 4벌·코칭 5종. 각각 좋아지는 방향과 잡음 폭이 코드에 박혀 있다.
...
- **표본이 작다.** 기본은 번들 1개(`live_qa_run.json`)·질문 2개. 한 발표에서 좋아진 게 다른 발표에서 나빠졌는지 못 본다.
  ...
  지금 `qa_eval_compare.py` 는 tag 의 **마지막 파일**만 비교한다 — 번들 N개 평균 비교는 아직 없다 (할 일 #3 이어서).
- **LLM-as-a-Judge 가 아직 코드에 없다.** rubric 7항목은 `qa-question-judge` 에이전트가 사람처럼 읽는다.
  숫자 rubric 이 자동화되면 `qa_eval.py` 의 summary 에 넣고 `PRIMARY` 표에 한 줄 추가하면 된다.
- 잡음 폭(특이도 ±0.5, 인용률 ±0.05)은 9/10 결과 다섯 개를 보고 정한 값이다. 표본이 늘면 다시 잰다.
