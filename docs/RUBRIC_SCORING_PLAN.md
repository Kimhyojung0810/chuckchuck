...
### 9-1. 실 LLM(solar) 확인 결과

모의 LLM만으로는 안 드러나던 문제 셋을 실 API에서 잡아 고쳤습니다.

| 증상 | 원인 | 고친 것 |
|---|---|---|
| 항목 절반이 "못 잼" | 모델이 판단 못 한 항목에 `score:0` + `evidence:""` 로 응답 | 시스템 프롬프트에 **점수 구간의 뜻**을 명시하고, "모르겠으면 0점 말고 번호를 아예 빼라"로 바꿈 |
| 근거 자리에 항목 설명이 그대로 | 모델이 프롬프트를 복사해 넣음 | `_normalize_llm` 이 채점표 문구가 섞인 근거를 버림 |
| 13·14(청중·목적)가 늘 0점 | **프롬프트에 청중·목표 시간이 아예 안 들어가고 있었다** | `Evidence.audience`·`duration_min` 추가, 없으면 "안 알려 줬어요"라고 명시 |

추가로 `score` 키가 없는 항목을 0으로 채우던 것을 **버리도록** 고쳤습니다 — 모델이 판단조차
안 한 항목이 "0점 = 아예 안 했다"로 둔갑하면 안 매긴 것보다 나쁜 거짓말이 됩니다.

**현재 실측 (IMU2CLIP 샘플, solar, 12초):** 77점 · LLM 15항목 중 11~12개 채점 ·
근거가 전부 실제 발화·슬라이드 인용.

**남는 한계 (정직하게 둘 것):**
- 시각자료 묶음(29·37·39)이 실행마다 흔들린다. 근거를 못 대면 버리므로 `basis` 는
  실전에서 대개 `partial` 이다. 이건 설계대로 동작하는 것이지 버그가 아니다.
- 6번(선행연구 인용)은 실제로 인용이 없는 발표에서 "0점"이 아니라 "못 잼"이 된다.
  근거 없는 점수를 안 쓰기로 한 대가다.
- **발표자료와 발화가 서로 다른 자료면 모델이 "내용이 없다"고 답한다.** 테스트할 때
  짝이 맞는 fixture 를 써야 한다 (`sample_transcript.json` + `sample_slidedoc.json`).
