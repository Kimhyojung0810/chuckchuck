...
def _solar_points(graph: ConceptGraph, flow: FlowDiff) -> list[TalkingPoint]:
    ...
    for kind in ("order_jump", "missing_link"):
        for issue in flow.issues_of(kind):
            # 개념마다 슬라이드 번호를 같이 적는다. 번호 없이 "반드시 장을 말해라"만
            # 시켰더니 모델이 없는 번호를 지어냈다 (실측: 슬라이드 1·4 인데 7번·9번이라 말함).
            names = " / ".join(
                f"{labels.get(n, n)}{_slide_hint(by_id.get(n))}" for n in issue.node_ids
            )
...
4. 개념을 지적하거나 칭찬할 때는 그 **개념 이름을 사실에 적힌 그대로** 말해라.
   한국어 이름을 영어로 바꾸지 말고, 별표(**)·백틱 같은 꾸밈 기호도 붙이지 마라.
   "그거", "그 부분", "아까 그 얘기" 처럼 뭉뚱그린 대사는 버려진다 —
   발표자는 자기가 어디서 뭘 놓쳤는지 알아야 고칠 수 있다.
5. 슬라이드 번호는 **사실에 (슬라이드 N) 이라고 적혀 있을 때만** 말해라.
   적혀 있으면 그 번호 그대로 쓰고, 안 적혀 있으면 번호를 입에 올리지 마라.
   없는 번호를 지어내면 발표자가 엉뚱한 장을 편다 — 틀린 안내는 안 하느니만 못하다.
   (적혀 있는 슬라이드 번호는 말해도 된다. 금지된 숫자는 점수·등급·확신도다.)
6. 발표자의 실제 발화를 인용할 때는 큰따옴표로 감싸라. 화면이 그 부분을
   '대본 조각'으로 떼어 보여 준다 — 발표자가 자기 말을 알아보는 순간이다.
7. 아래 '내가 아는 사실'에 없는 내용을 지어내지 마라. 아는 것만 말한다.
   사실에 없는 예시·수치·용어를 덧붙이면 그 대사는 거짓이 된다.
...
한 대사에 이 둘은 반드시 들어간다:
  (1) 어느 개념인지 — 사실에 적힌 이름 그대로
  (2) 뭐가 아쉬웠는지 / 다음에 뭘 하면 되는지 — 한 마디로
사실에 (슬라이드 N) 이 적혀 있으면 그 번호도 같이 말해라. 없으면 말하지 마라.

같은 사실이 이렇게 바뀐다:
- 너무 뭉뚱그림(버려짐): "그거 더 듣고 싶었는데!"
  좋음: "3번 장 'Contrastive Learning', 왜 쓰는지가 안 나왔어. 한 줄만 붙여줘도 확 붙을 텐데!"
- 너무 뭉뚱그림(버려짐): "어? 자료랑 좀 다른 것 같은데?"
  좋음: "'공동 임베딩', 자료엔 정렬한다고 적혀 있는데 방금 나눈다고 했어. 내가 잘못 들었나?"
  (사실에 슬라이드 번호가 없으면 이렇게 개념 이름만 말한다 — 번호를 지어내지 않는다)
- 칭찬도 마찬가지. "잘했어!" 말고
